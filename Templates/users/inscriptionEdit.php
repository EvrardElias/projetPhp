    ...
    <form method="post" action="">
        <fieldset>
            <legend><?php if(isset($_SESSION["user"])) : ?>Modification du profil<?php else : ?>Inscription<?php endif ?></legend>
                ...
                <div>
                    <label for="role">Restaurateur</label>
                    <input type="checkbox" <?= isset($_SESSION["user"]) ? 'disabled' : '' ?> class="form-control" id="role" name="role" <?= (isset($_SESSION["user"]) && $_SESSION["user"]->utilisateurRole === 'Restaurateur') ? 'checked' : '' ?> />
                </div>
                <div>
