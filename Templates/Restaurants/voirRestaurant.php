<div class="flexible justify-content-center">   
    <div class="cadreMenu">
        ...
        <p><span> <?php if(empty($etoiles)) : ?> Il n'y a pas d'étoiles pour l'instant <?php else : ?> <?php foreach($etoiles as $etoile) :?> <?= $etoile->etoileNombre ?> ★ <?php endforeach?> <?php endif ?></span> - <span> <?php if(empty($nourritures)) : ?> Il n'y pas de de type de nourriture pour l'instant <?php else : ?> <?php foreach($nourritures as $nourriture) :?> <?= $nourriture->nourritureType ?> <?php endforeach?> <?php endif ?></span></p>
